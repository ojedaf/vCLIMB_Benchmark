...
class CILSetTask:
    ...
    def get_inc_classes(self, data):
        classes = []
        if len(self.memory) > 0:
            for task in self.memory:
                classes.extend(list(task.keys()))
        classes.extend(list(data.keys()))
        return classes
    
    def __next__(self):
        data = self.set_tasks[self.current_task]
        classes_dict = self.get_inc_classes(data)
        current_task_dataset = TSNDataSet(self.path_frames, data, classes_dict, self.num_segments, self.new_length, 
        ...
        self.rehearsal_randomMethod(data, self.current_task)
    ...
    def get_multitask_memory(self):
        list_train_memory = []
        classes = self.get_inc_classes({})
        len_mem = len(self.memory)
        last_task = self.memory[len_mem-1]
        num_t_instance = 0
        for k, values in last_task.items():
            num_t_instance += len(values)
        batch_last_task = num_t_instance // 4
        if batch_last_task > self.batch_size:
            batch_last_task = self.batch_size
        print('batch_last_task: ',batch_last_task)
        all_rest_mem = {}
        for task in self.memory[:-1]:
            all_rest_mem = {**all_rest_mem, **task}
        
        task_dataset_multi = TSNDataSet(self.path_frames, all_rest_mem, classes, self.num_segments, self.new_length, 
                                        self.modality, self.transform, self.random_shift, self.test_mode, 
                                        self.remove_missing, self.dense_sample, self.twice_sample)
        task_dataloader_multi = DataLoader(task_dataset_multi, batch_size = self.batch_size, shuffle = self.shuffle, 
                                           num_workers = self.num_workers, pin_memory = self.pin_memory, drop_last = self.drop_last)
        
        task_dataset_last = TSNDataSet(self.path_frames, last_task, classes, self.num_segments, self.new_length, 
                                       self.modality, self.transform, self.random_shift, self.test_mode, 
                                       self.remove_missing, self.dense_sample, self.twice_sample)
        task_dataloader_last = DataLoader(task_dataset_last, batch_size = batch_last_task, shuffle = self.shuffle, 
                                          num_workers = self.num_workers, pin_memory = self.pin_memory, drop_last = self.drop_last)
            
        return task_dataloader_multi, task_dataloader_last
    ...
    def rehearsal_randomMethod(self, current_task, id_task):
        current_classes = current_task.keys()
        num_classes = self.get_num_classes() + len(current_classes)
        self.memory.append(current_task)
        if self.memory_size != 'ALL':
            num_instances_per_class = self.memory_size // num_classes
            for i, task in enumerate(self.memory):
                for class_n, elems in task.items():
                    random.shuffle(elems)
                    self.memory[i][class_n] = elems[:num_instances_per_class]
