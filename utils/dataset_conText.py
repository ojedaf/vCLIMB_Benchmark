...
class TSNDataSet(data.Dataset):
    def __init__(self, path_frames, data, classes=None, 
        ...
        self.classes_emb = {name:torch.from_numpy(model.encode(" ".join(re.findall('[A-Z][^A-Z]*', name)).lower())) for name in self.classes}
        ...
        for class_name, videos in self.data.items():
            for video_name in videos: 
                ...
                if num_frames >= self.num_segments:
                    item = [path_video, frames, num_frames, self.classes_label[class_name], self.classes_emb[class_name]]
    ...
    def get(self, record, indices):
        ...
        classes_emb = torch.stack(list(self.classes_emb.values()), dim =0)
        return process_data, record.class_emb, record.label, classes_emb
