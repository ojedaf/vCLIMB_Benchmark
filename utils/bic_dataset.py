...
class CILSetTask:
    ...
    def get_dataloader(self, data, batch_size = 1, memory = None):
        if memory != None:
            data = {**memory, **data}
    ...
    def __next__(self):
        ...
        if self.current_task == 0:
            train_train_dataset = TSNDataSet(self.path_frames, comp_data, None, self.num_segments, self.new_length, 
                                            self.modality, self.transform, self.random_shift, self.test_mode, 
                                            self.remove_missing, self.dense_sample, self.twice_sample)
            ...
            train_train_dataset = TSNDataSet(self.path_frames, train_train_data, None, self.num_segments, self.new_length, 
                                            self.modality, self.transform, self.random_shift, self.test_mode, 
                                            self.remove_missing, self.dense_sample, self.twice_sample)
            len_train_data = len(train_train_dataset.video_list)
            train_train_dataloader = DataLoader(train_train_dataset, batch_size = self.batch_size, shuffle = self.shuffle, 
            ...
            train_val_dataset = TSNDataSet(self.path_frames, train_val_data, None, self.num_segments, self.new_length, 
                                            self.modality, self.transform, self.random_shift, self.test_mode, 
                                            self.remove_missing, self.dense_sample, self.twice_sample)
        ...
        for class_name, videos in self.data.items():
            for video_name in videos: 
                ...
                if num_frames >= self.num_segments:
                    item = [id_vid, path_video, frames, num_frames, class2label[class_name]]
