    ...
    def get_valSet_by_taskNum(self, num_task):
        ...
        for i, data_i in enumerate(total_data):
            val_task_dataset = TSNDataSet(self.path_frames, data_i, classes, self.num_segments, self.new_length, 
                                              self.modality, self.transform, self.random_shift, self.test_mode, 
                                              self.remove_missing, self.dense_sample, self.twice_sample)
            self.classes_text = val_task_dataset.classes_text
...
class TSNDataSet(data.Dataset):
    def __init__(self, path_frames, data, classes=None, 
        ...
        self.classes_text = {name:'a photo of {}'.format(" ".join(re.findall('[A-Z][^A-Z]*', name)).lower()) for name in self.classes}
        self.classes_label = {name:i for i,name in enumerate(self.classes)}
        self.num_segments = num_segments
        self.new_length = new_length
        self.modality = modality
        self.transform = transform
        ...
        for class_name, videos in self.data.items():
            for video_name in videos: 
                ...
                if num_frames >= self.num_segments:
                    item = [path_video, frames, num_frames, self.classes_label[class_name], self.classes_text[class_name]]
...
#         class_text = list(np.repeat([record.class_text],self.num_segments))
        return process_data, record.class_text, record.label
