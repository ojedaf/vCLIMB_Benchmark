...
def main():
    ...
    dict_conf = yaml.load(conf_file,Loader=SafeLoader)
...
def train_loop(model, optimizer, train_cilDatasetList, val_cilDatasetList, test_cilDatasetList):
    ...
    for j in range(num_tasks):
        ...
        if j==0:
            mean_arr = 0
        if j>0:
            new_embs = model.get_emb(val_cilDatasetList)
            mean_arr= model.sem_drift(old_embs, new_embs)
        ...
        if type_sampling == 'icarl':
            ...
            for _, _, videos, _, labels in train_eval_loader:
                ...
                preds = model.classify(videos, mean_arr)
        ...
        with experiment.validate():
            total_acc_val = model.final_validate(val_cilDatasetList, j, experiment, mean_arr,'val')
            print('Val Accuracy: %d %%' % total_acc_val)
        
        # Init testing.
        if not is_activityNet:
            with experiment.test():
                total_acc_test = model.final_validate(test_cilDatasetList, j, experiment, mean_arr,'test')
